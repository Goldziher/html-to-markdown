"""Integration tests for CLI v1 compatibility.

These tests actually execute the CLI binary with v1 flags to ensure
end-to-end compatibility works correctly.
"""
...
class TestCLIV1Integration:
    ...
    def run_cli(self, args: list[str], input_html: str = "") -> tuple[str, str, int]:
        """Run CLI via Python module (with v1 translation) and return (stdout, stderr, returncode).

        Args:
            args: Command line arguments
            input_html: HTML to pass via stdin
    ...
    def test_basic_conversion_stdin(self) -> None:
        """Test basic conversion from stdin."""
    ...
    def test_file_conversion_with_v1_flags(self) -> None:
        """Test file conversion with v1 flags."""
    ...
    def test_preprocess_html_flag_translation(self) -> None:
        """Test --preprocess-html flag is translated to --preprocess."""
    ...
    def test_unsupported_v1_flag_strip_raises_error(self) -> None:
        """Test that --strip flag raises an error."""
        html = "<a href='#'>link</a>"
        _stdout, stderr, returncode = self.run_cli(["-", "--strip", "a"], html)
        assert returncode != 0
        assert "--strip option is not supported in v2" in stderr

    def test_unsupported_v1_flag_convert_raises_error(self) -> None:
        """Test that --convert flag raises an error."""
        html = "<a href='#'>link</a>"
        _stdout, stderr, returncode = self.run_cli(["-", "--convert", "a"], html)
        assert returncode != 0
        assert "--convert option is not supported in v2" in stderr

    def test_complex_v1_command(self) -> None:
        """Test complex command with many v1 flags."""
        html = """
        <html>
    ...
    def test_sub_sup_symbols(self) -> None:
        """Test subscript/superscript symbol options."""
...
class TestCLIV1ErrorHandling:
    """Test error handling for CLI v1 compatibility."""

    def run_cli(self, args: list[str], input_html: str = "") -> tuple[str, str, int]:
        """Run CLI via Python module and return (stdout, stderr, returncode)."""
        result = subprocess.run(
            ["python", "-m", "html_to_markdown", *args],
            input=input_html,
            capture_output=True,
